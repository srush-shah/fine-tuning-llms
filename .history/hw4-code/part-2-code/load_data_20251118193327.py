...
def format_schema_summary(schema):
    """Format schema into a concise summary string for inclusion in prompts."""
    ...
    for table_name in key_tables:
        if table_name in schema['ents']:
            ...
            if isinstance(columns, dict):
                # Only include key columns (first 10 to keep it concise)
                col_names = list(columns.keys())[:10]
                tables_info.append(f"{table_name}({','.join(col_names)})")
    
    # Add other tables if space allows
    for table_name, columns in schema['ents'].items():
        if table_name not in key_tables and isinstance(columns, dict):
            if len(tables_info) < 15:  # Limit total tables
...
try:
    ...
    if len(_schema_summary) > 300:
        _schema_summary = _schema_summary[:300] + "..."
...
class T5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if _schema_summary:
            source_text = f"{TASK_PREFIX}Schema: {_schema_summary} | {nl_query}"
        else:
