...
def format_schema_summary(schema):
    """Format schema into a concise, focused summary string for inclusion in prompts."""
    if not schema or 'ents' not in schema:
        return ""
    
    # Focus on most commonly used tables and their key columns
    # Based on analysis of the SQL queries, these are the key tables
    key_tables_info = {
        'flight': ['flight_id', 'from_airport', 'to_airport', 'departure_time', 'arrival_time', 'flight_days', 'airline_code'],
        'airport_service': ['airport_code', 'city_code', 'airline_code'],
        'city': ['city_code', 'city_name', 'state_code'],
        'airport': ['airport_code', 'airport_name'],
        'days': ['days_code', 'day_name'],
        'date_day': ['year', 'month_number', 'day_number', 'day_name'],
        'airline': ['airline_code', 'airline_name'],
    }
    
    # Build concise schema summary
    tables_info = []
    for table_name, key_cols in key_tables_info.items():
        if table_name in schema['ents']:
            # Use key columns we specified, or fall back to schema columns
            available_cols = list(schema['ents'][table_name].keys())
            cols_to_use = [c for c in key_cols if c in available_cols] or available_cols[:8]
            tables_info.append(f"{table_name}({','.join(cols_to_use)})")
    
    # Format as compact schema description
    schema_text = "Tables: " + " | ".join(tables_info)
    return schema_text
