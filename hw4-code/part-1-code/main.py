...
def create_augmented_dataloader(args, dataset):
    ...
    # Sample 5k random examples from training set and transform them
    sample_size = min(5000, len(original_train))
    sampled_dataset = original_train.shuffle(seed=42).select(range(sample_size))
    transformed_dataset = sampled_dataset.map(custom_transform, load_from_cache_file=False)
    
    # Combine original training set with transformed examples
    from datasets import concatenate_datasets
    augmented_dataset = concatenate_datasets([original_train, transformed_dataset])
...
if __name__ == "__main__":
    ...
    # Load only train and test splits (skip unsupervised split which may not be available)
    try:
        dataset = load_dataset("imdb")
    except Exception as e:
        if "unsupervised" in str(e) or "ExpectedMoreSplits" in str(type(e).__name__):
            # Workaround: Use builder and modify info to skip unsupervised split verification
            from datasets import load_dataset_builder, DatasetDict
            builder = load_dataset_builder("imdb")
            # Modify the dataset info to remove unsupervised from expected splits
            if hasattr(builder.info, 'splits') and builder.info.splits:
                # Create a new splits dict without unsupervised
                new_splits = {k: v for k, v in builder.info.splits.items() if k != "unsupervised"}
                builder.info.splits = new_splits
            # Download and prepare
            builder.download_and_prepare()
            # Load the dataset
            dataset = builder.as_dataset()
            # Ensure we only have train and test
            if "unsupervised" in dataset.keys():
                dataset = DatasetDict({k: v for k, v in dataset.items() if k != "unsupervised"})
        else:
            raise
